   ...
   void _startPeriodicChatUpdate() {
    _timer = Timer.periodic(Duration(seconds: 1), (timer) async {
      ...
      List<Map<String, dynamic>> chats =
          dynamicChats.map((chat) => chat as Map<String, dynamic>).toList();
      List<Map<String, dynamic>> chatsGroups = dynamicChatsGroups
          .map((chatGroup) => chatGroup as Map<String, dynamic>).toList();
...
Future<void> _loadChatsWithNewMessages(List<Map<String, dynamic>> chats) async {
  print("Cargando chats con mensajes no leídos");
  getCallerInfo();
  for (var chat in chats) {
    String chatName = chat['ELEMENTO_NUM'].toString();
    String nombreCompleto = chat['NOMBRE_COMPLETO'].toString().replaceAll(' ', ''); // Eliminamos espacios en blanco
...
return ListView.builder(
  ...
  itemBuilder: (context, index) {
    if (index < filteredChats.length) {
      ...
      bool hasUnreadMessages = chatsWithNewMessages.contains(chat['NOMBRE_COMPLETO'].toString().replaceAll(' ', ''));
        ...
        children: [
          Container(
            color: isRequestCall
                ? Colors.blue
                : hasUnreadMessages
                    ? Color.fromARGB(255, 204, 204, 204) // Fondo gris si hay mensajes no leídos
                    : Colors.transparent,
            child: ListTile(
              leading: Image.asset(
              ...
              title: Text(
                ...
                style: TextStyle(
                  ...
                  color: isRequestCall
                      ? Colors.white
                      : hasUnreadMessages
                          ? Colors.grey[800] // Texto gris oscuro si hay mensajes no leídos
                          : Colors.black,
                ),
              ),
              subtitle: Text(
      ...
      bool hasUnreadMessagesGroup = chatsWithNewMessages.contains(chatGroup['NOMBRE_COMPLETO'].toString().replaceAll(' ', ''));
